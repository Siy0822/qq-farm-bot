...
- **个人生涯统计弹窗（新增）**：点击概览页微信头像弹出生涯面板，展示头像 / 昵称 / 等级 / 经验 / 角色编号，以及「历史累计收获」「累计摘取好友作物」两项统计与收获明细网格（含前三名标牌）。后端新增 `gamepb.careerpb.CareerService/CareerInfoGet` 与 `GET /api/career`。
- **好友页面崩溃修复（重要）**：`admin.js` 注册好友路由时漏传 `getAccountIdFromRequest` 等依赖，打开「好友」页面会抛出 `TypeError` 使整个 Node 进程退出、容器反复重启、登录掉线、账号自动停止；已补全依赖并加 `unhandledRejection` 全局守卫，单请求异常不再拖垮全服。
