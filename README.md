...
- **「经验满只帮护主犬」修复**：原判定依赖服务端恒为 `0` 的 `day_ex_times_lt`，导致该模式形同虚设、无差别帮助所有人。改为基于「帮好友后本号总经验是否增长」的增量判定——连续多次帮助成功但经验未增、且当日已帮过若干次，即判定经验已满，自动切换到仅帮护主犬好友；该状态写入配置、跨重启持久化，并在跨日时自动恢复。同时修复了「巡逻中途触发切换后本轮剩余普通好友仍被无差别帮助」的泄漏，以及因容器内 `FARM_ACCOUNT_ID` 为空导致该状态无法持久化的问题。
